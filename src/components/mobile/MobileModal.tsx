        ...
        {(title || showCloseButton) && (
          <div className="flex items-center justify-between px-4 py-3 border-b border-border shrink-0">
            ...
            {showCloseButton && (
              <button
                onClick={onClose}
                className="p-2 -mr-2 text-muted-foreground hover:text-text hover:bg-surface rounded-full transition-colors"
...
export const BottomSheet: React.FC<BottomSheetProps> = ({ isOpen, onClose, title, children }) => {
  ...
  if (isMobile) {
    return (
      ...
      >
        <div
          className="fixed inset-x-0 bottom-0 z-50 bg-background flex flex-col animate-in slide-in-from-bottom duration-300"
  ...
  return (
    ...
    >
      <div
        className="bg-background rounded-2xl max-h-[80vh] w-full max-w-lg mx-4 overflow-hidden flex flex-col animate-in zoom-in-95 duration-200"
        onClick={(e) => e.stopPropagation()}
      >
        {title && (
          <div className="flex items-center justify-between px-4 py-3 border-b border-border shrink-0">
  ...
  return (
    ...
    >
      <div className="p-4">
        ...
        <div className="space-y-2">
          {actions.map((action, index) => {
            const variantClasses = {
              default: 'text-text hover:bg-surface',
        ...
        <button
          onClick={onClose}
          className="w-full mt-4 px-4 py-3 text-base font-semibold text-text bg-surface hover:bg-border/30 rounded-xl transition-colors"
        >
          {cancelLabel}
        </button>
      </div>
